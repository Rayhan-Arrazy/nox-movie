...
<div class="min-h-screen pb-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div class="mb-6 animate-fade-in-up">
            ...
            <p class="text-sm text-[#7a6e60]">Discover your next favorite film</p>
        </div>

        <!-- Search & Filters -->
        <div class="bg-[#1a1714] rounded-2xl p-4 sm:p-5 mb-6 border border-white/5 animate-fade-in-up"
            style="animation-delay:80ms">
            <div class="flex flex-col sm:flex-row gap-3">
                <form onsubmit="handleBrowseSearch(event)" class="flex-1 relative">
                    <i data-lucide="search"
                        class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#7a6e60]"></i>
                ...
                <div class="flex items-center gap-2">
                    <button onclick="setGridSize('normal')"
                        class="p-2.5 rounded-xl bg-[rgba(232,184,75,0.1)] text-[#e8b84b] border border-[rgba(232,184,75,0.2)] transition-all"
                        title="Normal grid">
                        <i data-lucide="grid-3x3" class="w-4 h-4"></i>
                    </button>
                    <button onclick="setGridSize('large')"
                        class="p-2.5 rounded-xl bg-[#242019] text-[#7a6e60] border border-white/5 hover:text-white transition-all"
                        title="Large grid">
            ...
            <div class="mt-3 flex flex-wrap gap-2 items-center">
                <i data-lucide="filter" class="w-3.5 h-3.5 text-[#7a6e60]"></i>
                <?php foreach ($genres as $genre): ?>
                    <button onclick="filterGenre('<?= esc($genre) ?>')" data-genre="<?= esc($genre) ?>"
                        class="genre-btn cat-tab px-3.5 py-1.5 rounded-full text-[11px] font-medium text-[#7a6e60]">
                        <?= esc($genre) ?>
                    </button>
                <?php endforeach; ?>
            </div>
        ...
        <div class="flex items-center justify-between mb-5">
            <p id="results-count" class="text-xs text-[#7a6e60]">Loading...</p>
        </div>
